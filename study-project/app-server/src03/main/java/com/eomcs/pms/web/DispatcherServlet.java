...
public class DispatcherServlet extends HttpServlet {
  ...
  public void init() throws ServletException {
    ...
    try {
      ...
      File conrtollerFilePath = Resources.getResourceAsFile(controllerPackageName.replace(".", "/"));
      //      System.out.println(conrtollerFilePath.getCanonicalPath());

      // 해당 디렉토리에서 .class 파일을 찾는다.
      File[] controllerFiles = conrtollerFilePath.listFiles(
      ...
      for (File f : controllerFiles) {
        ...
        if (anno != null) {
          ...
          // 3) 생성자를 호출하기 전에 파라마터 타입에 해당하는 아규먼트를 준비한다.
          // - 아규먼트를 담을 배열 준비
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      String viewName = (String) requestHandler.invoke(controller, request, response);
